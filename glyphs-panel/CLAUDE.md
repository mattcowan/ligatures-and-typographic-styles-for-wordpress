...
- `assets/js/lib/` — hand-written ES5, no build step; each file is UMD-lite (`window.*` + CommonJS export for Jest):
  ...
  - `search.js`, `window-grid.js`, `insertion.js` — search/filtering, virtualized ARIA grid, and insertion via the `typost-insert-content` CustomEvent (indexed alternates use `data-feature-settings`, e.g. `"salt" 2`). The alternates view's **base cell** passes the view's feature tags as `clearTags`: any of them active in context are written as `"tag" 0` in `data-feature-settings` (kept context tags follow at 1, and `data-features` lists only the kept tags), because an alternate that comes from a block-level feature or paragraph style survives a plain inline-format reset (rule 6 in `insertion.js`).
- `glyphs-modal.js` / `editor.js` — the modal UI + editor buttons injected at `typost_inline_before_features` and `typost_qft_before_features`. The modal renders core's **word-boundary accessibility notice** at its top from `context.accessibility` (supplied only on a toolbar launch — opened from inside an editor modal, that modal shows the notice itself), and offers the conversion through the `typost-convert-to-block` CustomEvent, closing afterwards because the conversion replaces the very block the panel is pointed at. `editor.js` also registers an **optional block toolbar button** (v1.3+) on core's `typost_editor_toolbar_buttons` filter when `typost_glyphs_toolbar_button` is on (default off; setting rendered into core's Options tab via `typost_admin_options_rows`, flag exposed as `typostData.glyphsPanel.toolbarButton`). Its `GlyphsIcon` is a swash "G" on a 256×256 grid, drawn as one of a set with core's swash "T" and the Paragraph Styles swash "P" so the toolbar group shares a cap height and baseline; the source lives in core at `assets/images/icons/toolbar-g.svg`. The toolbar context uses the *same field names* as the existing hook snapshots (`clientId`/`capturedSelection`, `savedSelectionStart`/`savedSelectionEnd`/`selectedText`), so `openModal(context.source, context)` works unchanged; it additionally reads `context.state` (in preference to the shared state filter) and threads `context.reopenHost` into the `typost_glyphs_panel_closed` payload so a toolbar launch does not pop open a host modal on close.
